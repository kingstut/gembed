...
def n_clip_loss(similarity: torch.Tensor) -> torch.Tensor:
    dim1, dim2 = similarity.shape 
    dim_1_loss = []
    dim_2_loss = []
    for i in range(dim1):
        dim_1_loss.append(contrastive_loss(similarity[i]))
    
    for i in range(dim2):
        dim_2_loss.append(contrastive_loss(similarity.t()[i]))  

    dim_1_loss = sum(dim_1_loss)/ len(dim_1_loss)
...
class n_CLIPModel(nn.Module):
    config_class = n_CLIPConfig

    def __init__(self, config: n_CLIPConfig):
        super().__init__(config)

        n_config = config.configs
        ...
        for conf in n_config:
            self.n_embed_dim.append(conf.hidden_size)

            if isinstance(conf, CLIPTextConfig): 
                self.n_model.append(CLIPTextTransformer(conf))
            else: 
                self.n_model.append(CLIPVisionTransformer(conf))

            self.n_projection.append(nn.Linear(conf.hidden_size, self.projection_dim, bias=False))
    ...
    def forward(
        ...
        # normalized features
        n_embeds = [x / x.norm(p=2, dim=-1, keepdim=True) for x in n_embeds]
